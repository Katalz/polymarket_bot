...
TARGET_CRYPTOS = ["sol"]
LOG_FILE_PREFIX = "straddle_strategy_log_sol"
STATE_FILE = "gabagool_lite_state_sol.json"
...
def discover_market(is_test_mode=False):
    """
    Discover active SOL 15m markets.
    Try direct API approach first, fallback to constructed slugs.
    """
    print("[DISCOVER] Searching for SOL 15m markets...")
    ...
    try:
        ...
        for m in markets:
            slug = m.get("slug", "")
            if "sol" not in slug.lower() or "15m" not in slug.lower():
                continue

            print(f"[DISCOVER] Found SOL 15m market: {slug}")
    ...
    if is_test_mode:
        ...
        test_slug = f"sol-updown-15m-{current_interval_start}"
    ...
    print("[DISCOVER] No active SOL 15m markets found")
    return None

# ---------------- UTILS: SUMMARY LOGGING ---------------- #

def log_market_summary(strategy, market_info):
    """Logs a one-line summary of the market result to market_summaries_sol.csv."""
    summary_file = "market_summaries_sol.csv"
    if not strategy: 
        return
    ...
    while True:
                ...
                if (now - strategy.last_fill_ts) > strategy.SYNC_GRACE_PERIOD:
                    ...
                    if up_bal is not None and down_bal is not None:
                        ...
                        if abs(old_up - up_bal) > 0.01 or abs(old_down - down_bal) > 0.01:
                            print(f"[SOL-BOT] [SYNC] Inventory updated: {old_up}/{old_down} -> {up_bal}/{down_bal}")
            except Exception as e:
        ...
        if strategy.state.value == "IDLE":
            ...
            if enter_result["action"] == "ENTER":
                ...
                if "ORDER" not in reason: 
                     ...
                     print(f"[SOL-BOT] [STATUS] Spread: {sum_px:.4f} (Avg: {ewma_s}) | Inv: {inv_s} | Reason: {reason}")
        ...
        if action_type == "TIMEOUT_CUT":
            ...
            if strategy.down_order_id and not strategy.down_filled:
             ...
             if not DRY_RUN:
                 # Place IMMEDIATE OR CANCEL or just Limit Taker
                 # Using place_limit_maker with allowance for taker? 
                 # No, we need a taker order function.
                 # place_limit_maker forces POST_ONLY usually via client or args?
                 # pm_wrapper.place_limit_maker is maker only??
                 # Let's check pm_wrapper. It calls create_and_post_order.
                 # We need to send a standard order (not post only).
                 # ClobClient usually defaults to aggressive if not specified as POST_ONLY?
                 # Actually place_limit_maker likely sets some flags.
                 # We'll rely on a new method place_market_buy or similar if exists, 
                 # or just place a limit order at the ask.
                 
                 # NOTE: Using a standard Limit order at Ask Price should fill as Taker.
                 # We'll use pm_wrapper.place_limit_maker but ignore the name implies Maker.
                 # Actually, let's verify if place_limit_maker enforces maker.
                 # If it just posts an order, it might cross.
                 
                 # Simplification: Use existing place_limit_maker but with permissive price.
                 # Ideally rename or add 'place_limit_taker'.
                 ...
                 if taker_oid:
                     ...
                     strategy.state = strategy.state.STRADDLE_COMPLETE # Assume success or handle filling?
                     # Ideally we wait for fill confirmation.
                     time.sleep(1.0)
                     ...
                     if filled:
                          ...
                          # Retry or go to Flatten?
                          # Let it retry next tick or fall to CUT.
             else:
        ...
        if strategy.state.value == "FLATTENING":
            ...
            if leg_to_sell and qty_to_sell > 0:
                ...
                if unwind_order_id:
                    ...
                    if _is_filled:
                        print(f"[FLATTENING] Sell confirmed at {avg_px:.4f}.")
...
def run_verify_mode():
    ...
    try:
        ...
        # Test parsing
        slug = "sol-updown-15m-1765836900"
