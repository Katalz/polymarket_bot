...
class StraddleBuyHoldStrategy:
    """
    Buy/Hold Only Strategy (Strict Invariants).
    - No Selling.
    ...
    def __init__(self, slug: str, has_traded: bool = False, size_optimizer=None, bankroll_usd: float = 10000.0, initial_state: dict = None):
        ...
        # self.actual_size = 0.0 # Moved above
        ...
        # self.actual_size = 0.0 # Last entry size # Moved above
        ...
        print(f"[BUY_HOLD] Init {self.slug}. Cap={self.MAX_TOTAL_SHARES}")
    ...
    def maybe_enter(self, up_bid, up_ask, down_bid, down_ask, time_remaining, available_balance=None, liquidity_depth=None, test_mode=False):
        ...
        # 1. TIME FREEZE GATE
        # If < 3 min, we freeze. No new orders. Deadlock allowed.
        if time_remaining < 180:
            return {'action': 'IDLE', 'reason': f'TIME_FREEZE_GATE (<3m)'}
        ...
        # 3. IMBALANCE CHECK -> PRIORITY REBALANCE
        imbalance = self.filled_up_shares - self.filled_down_shares
        ...
        if is_unbalanced:
            ...
            # Deadlock Relief Logic
            # If Time < 5 min, we tolerate up to 1.02. Else 1.00.
            max_tolerated_sum = 1.02 if time_remaining < 300 else 1.00
            
            if synthetic_sum > max_tolerated_sum:
                 return {'action': 'IDLE', 'reason': f'REBALANCE_BLOCKED_COST: Synth {synthetic_sum:.3f} > {max_tolerated_sum} (Cost:{avg_existing:.3f} + Ask:{cost_check_px:.3f})'}
        ...
        # Condition: Time > 8 min (480s)
        if time_remaining < 480:
             return {'action': 'IDLE', 'reason': 'TIME_REINFORCE_GATE (<8m)'}
