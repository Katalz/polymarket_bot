    ...
    while True:
        ...
        if not current_market:
            ...
            if m:
                ...
                # Check persistence
                initial_state = persisted_state.get(slug, {})
                if isinstance(initial_state, bool):
                     # Legacy format
                     initial_state = {"has_traded": initial_state}
                
                has_traded = initial_state.get("has_traded", False)
                if has_traded:
                    print(f"[STATE] Market {slug} already traded (recorded in persistence)")
        ...
        # Get fresh orderbooks and inventory
        try:
            ...
            if not DRY_RUN:
                ...
                try:
                    ...
                    if (now - strategy.last_fill_ts) > strategy.SYNC_GRACE_PERIOD:
                        up_bal = pm_wrapper.get_balance(current_market["up_token_id"])
                        down_bal = pm_wrapper.get_balance(current_market["down_token_id"])
                        if up_bal is not None and down_bal is not None:
                            ...
                            if abs(old_up - up_bal) > 0.01 or abs(old_down - down_bal) > 0.01:
                                print(f"[SYNC] Inventory updated from API: {old_up}/{old_down} -> {up_bal}/{down_bal}")
                except Exception as e:
                    print(f"[!] Sync error: {e}")
        except Exception as e:
            print(f"[!] Orderbook/Balance error: {e}")
            time.sleep(1)
            continue
        ...
        if strategy.state.value in ["IDLE", "ONE_LEG_INVENTORY"]:
            ...
            if enter_result["action"] == "ENTER":
                ...
                # More frequent status but slightly throttled or only on skips
                if "ORDER" not in reason: # Don't spam if orders are open
                     ...
                     print(f"[STATUS] Spread: {sum_px:.4f} (Avg: {ewma_s}) | Inv: {inv_s} | Reason: {reason}")
        ...
        if action_type == "TIMEOUT_CUT":
            ...
            if strategy.down_order_id and not strategy.down_filled:
             ...
             print(f"[BTC-BOT] [TAKER-COMPLETE] Attempting to buy {qty} of {leg} @ {price:.3f} to complete straddle.")
             ...
             if not DRY_RUN:
                 ...
                 if taker_oid:
                     print(f"[BTC-BOT] [TAKER-COMPLETE] Order {taker_oid} placed. Waiting for fill...")
                     time.sleep(1.0)
                     filled, _, _ = pm_wrapper.check_order_status(taker_oid)
                     if filled:
                          print("[BTC-BOT] [TAKER-COMPLETE] SUCCESS. Straddle completed.")
                          ...
                          print("[BTC-BOT] [TAKER-COMPLETE] Failed to fill immediately. Cancelling...")
                          pm_wrapper.cancel_order(taker_oid)
             else:
                 print(f"[BTC-BOT] [TAKER-COMPLETE] (DRY) Would buy {qty} {leg} @ {price}")
                 strategy.state = strategy.state.STRADDLE_COMPLETE
        ...
        if strategy.state.value == "FLATTENING":
            ...
            if leg_to_sell and qty_to_sell > 0:
                ...
                if unwind_order_id:
                    ...
                    time.sleep(1.0) 
                    is_filled, avg_px, _ = pm_wrapper.check_order_status(unwind_order_id)
                    if is_filled:
