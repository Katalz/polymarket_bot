...
# Note: SafeBuyOnlyClientWrapper is imported above. Inline SafeClientWrapper removed.
...
TARGET_CRYPTOS = ["sol"]
LOG_FILE_PREFIX = "rebuild_only_log"
STATE_FILE = "state/sol_rebuild.json"
...
# EXECUTION CONFIG
# Hardcoded to rebuild_only for this runner
EXECUTION_MODE = "rebuild_only"
...
def run():
    ...
    if not DRY_RUN and pk:
        try:
            ...
            real_wrapper = PolymarketClientWrapper(client)
            pm_wrapper = SafeBuyOnlyClientWrapper(real_wrapper)
        ...
        real_wrapper = PolymarketClientWrapper(None)
        pm_wrapper = SafeBuyOnlyClientWrapper(real_wrapper)
    ...
    while True:
        ...
        if not current_market:
            ...
            if m:
                ...
                # Initialize Strategy for Rebuild Only
                print(f"[INIT] Using REBUILD-ONLY strategy for {slug}")
                strategy = StraddleRebuildStrategy(
                    slug=slug,
        ...
        if action_type == "TIMEOUT_CUT":
            ...
            if strategy.down_order_id and not strategy.down_filled:
             ...
             print(f"[SOL-BOT] [TAKER-COMPLETE] Attempting to buy {qty} of {leg} @ {price:.3f} to complete straddle.")
             ...
             if not DRY_RUN:
                 ...
                 if taker_oid:
                     print(f"[SOL-BOT] [TAKER-COMPLETE] Order {taker_oid} placed. Waiting for fill...")
                     time.sleep(1.0)
                     filled, _, _ = pm_wrapper.check_order_status(taker_oid)
                     if filled:
                          print("[SOL-BOT] [TAKER-COMPLETE] SUCCESS. Straddle completed.")
                          ...
                          print("[SOL-BOT] [TAKER-COMPLETE] Failed to fill immediately. Cancelling...")
                          pm_wrapper.cancel_order(taker_oid)
             else:
                 print(f"[SOL-BOT] [TAKER-COMPLETE] (DRY) Would buy {qty} {leg} @ {price}")
                 strategy.state = strategy.state.STRADDLE_COMPLETE
